...
cookie = '' # grab from POST request header in devtools
referrer = 'https://mint.intuit.com/transaction.event' # grab from POST request header in devtools
token = '' # grab from POST request form body in devtools
...
for row in csv_object:
	...
	curl2 = "curl \'https://mint.intuit.com/updateTransaction.xevent\'"
	curl_command = "curl \'https://mint.intuit.com/updateTransaction.xevent\'" + curl_line 
	...
	form_p1 = "\'cashTxnType=on&mtCheckNo=&tag" + tag1 + "=0&tag" + tag2 + "=0&tage" + tag3 + "=0&"
	...
	"""
	print(curl_input)
	curl_output = str(os.system(curl2)) # use os sytem to run a curl request submitting the form post
